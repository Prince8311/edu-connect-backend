...
if ($requestMethod === 'POST') {
    ...
    if ($type === 'week') {
        ...
        $updateStmt = $conn->prepare("UPDATE time_table SET status = ?, `classroom_id` = CONCAT(TRIM(`class`), TRIM(`section`), UPPER(LEFT(TRIM(`day`), 1)), UPPER(LEFT(TRIM(`subject`), 1)), LPAD(FLOOR(RAND() * 10000), 4, '0')) WHERE inst_id = ? AND `class` = ? AND `section` = ? AND status = 0");
        if (!$updateStmt) {
    ...
    $updateStmt = $conn->prepare("UPDATE time_table SET status = ?, `classroom_id` = CONCAT(TRIM(`class`), TRIM(`section`), UPPER(LEFT(TRIM(`day`), 1)), UPPER(LEFT(TRIM(`subject`), 1)), LPAD(FLOOR(RAND() * 10000), 4, '0')) WHERE inst_id = ? AND `class` = ? AND `section` = ? AND TRIM(LOWER(day)) = ? AND status = 0");
    if (!$updateStmt) {
